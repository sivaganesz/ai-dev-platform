...
import { useUpdateWorkflow, type Workflow } from "../hooks/use-workflows-data";
...
export function EditWorkflowModal({ open, onClose, workflow }: Props) {
  const updateWorkflow = useUpdateWorkflow();
  const { data: projects = [] } = useProjectsData();
  const [form, setForm] = useState<Partial<Workflow>>(workflow);
  ...
  const handleSubmit = () => {
    updateWorkflow.mutate({ id: workflow.id, ...form }, { onSuccess: onClose });
  };
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-lg">
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={updateWorkflow.isPending}>
